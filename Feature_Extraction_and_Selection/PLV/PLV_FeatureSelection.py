"""
PLV Feature Selection and Model Sweep — all subjects.
...
Views built from PLV's (n_trials, 3, 406) shape (406 = channel pairs):
  "theta", "alpha", "beta"   -> each (n_trials, 406)
  "all_bands_flat"           -> (n_trials, 1218), all bands concatenated
...
Outputs (per subject, in CSV_Results_PLV/):
  PLV_Sub{sid}_AllConfigurations.csv  -> every (view, selector, classifier, k) result
  PLV_Sub{sid}_Top10_LOO.csv          -> final top-10 configurations with LOO accuracy
"""
...
FEATURES_PATH_TEMPLATE = "features/PLV_Sub{sid}.npy"
Y_PATH_TEMPLATE = "data/processed/y{sid}_InnerSpeech_Task.npy"

BAND_NAMES = ["theta", "alpha", "beta"]  # must match band order used in feature extraction

SAVE_DIR = "CSV_Results_PLV"
...
def build_views(PLV):
    """PLV: (n_trials, n_bands, n_pairs). Returns per-band 2D views plus one concatenated view."""
    n_trials, n_bands, n_pairs = PLV.shape
    views = {}
    for bi, name in enumerate(BAND_NAMES[:n_bands]):
        views[name] = PLV[:, bi, :]
    views["all_bands_flat"] = PLV.reshape(n_trials, -1)
    return views
...
def run_stage1(views, y, subject_name):
    ...
    df_all.to_csv(os.path.join(SAVE_DIR, f"PLV_{subject_name}_AllConfigurations.csv"), index=False)
...
def run_stage2(top10, views, y, subject_name):
    ...
    df_stage2.to_csv(os.path.join(SAVE_DIR, f"PLV_{subject_name}_Top10_LOO.csv"), index=False)
    return df_stage2
...
def subject_already_done(subject_name):
    path = os.path.join(SAVE_DIR, f"PLV_{subject_name}_Top10_LOO.csv")
    return os.path.exists(path) and not FORCE_RERUN


if __name__ == "__main__":
    summary_path = os.path.join(SAVE_DIR, "PLV_all_subjects_summary.csv")
    summary_rows = []
    ...
    for sid in SUBJECT_IDS:
        ...
        try:
            t0 = time.time()
            PLV = np.load(FEATURES_PATH_TEMPLATE.format(sid=sid))
            y = np.load(Y_PATH_TEMPLATE.format(sid=sid))

            assert PLV.shape[0] == y.shape[0], "trial count mismatch between PLV and y"
            assert not np.isnan(PLV).any(), "PLV contains NaN"
            assert not np.isinf(PLV).any(), "PLV contains Inf"
            assert PLV.ndim == 3, f"expected 3D PLV features (n_trials, n_bands, n_pairs), got ndim={PLV.ndim}"

            views = build_views(PLV)
            print(f"  PLV shape={PLV.shape}, y shape={y.shape}, views={ {n: v.shape for n, v in views.items()} }")
