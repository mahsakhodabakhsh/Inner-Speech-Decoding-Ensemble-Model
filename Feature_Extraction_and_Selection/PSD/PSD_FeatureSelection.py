...
Views built from PSD's (n_trials, 3, 29) shape:
  "theta", "alpha", "beta"   -> each (n_trials, 29)   (29 = channels)
  "all_bands_flat"           -> (n_trials, 87), all bands concatenated
...
Outputs (per subject, in CSV_Results_PSD/):
  PSD_Sub{sid}_AllConfigurations.csv  -> every (view, selector, classifier, k) result
  PSD_Sub{sid}_Top10_LOO.csv          -> final top-10 configurations with LOO accuracy
"""
...
FEATURES_PATH_TEMPLATE = "features/PSD_Sub{sid}.npy"
...
def build_views(PSD):
    """PSD: (n_trials, n_bands, n_channels). Returns per-band 2D views plus one concatenated view."""
    n_trials, n_bands, n_ch = PSD.shape
    views = {}
    for bi, name in enumerate(BAND_NAMES[:n_bands]):
        views[name] = PSD[:, bi, :]
    views["all_bands_flat"] = PSD.reshape(n_trials, -1)
    return views
...
def run_stage1(views, y, subject_name):
    ...
    df_all.to_csv(os.path.join(SAVE_DIR, f"PSD_{subject_name}_AllConfigurations.csv"), index=False)
...
def run_stage2(top10, views, y, subject_name):
    ...
    df_stage2.to_csv(os.path.join(SAVE_DIR, f"PSD_{subject_name}_Top10_LOO.csv"), index=False)
    return df_stage2
...
def subject_already_done(subject_name):
    path = os.path.join(SAVE_DIR, f"PSD_{subject_name}_Top10_LOO.csv")
    return os.path.exists(path) and not FORCE_RERUN


if __name__ == "__main__":
    summary_path = os.path.join(SAVE_DIR, "PSD_all_subjects_summary.csv")
    summary_rows = []
    ...
    for sid in SUBJECT_IDS:
        ...
        try:
            t0 = time.time()
            PSD = np.load(FEATURES_PATH_TEMPLATE.format(sid=sid))
            y = np.load(Y_PATH_TEMPLATE.format(sid=sid))

            assert PSD.shape[0] == y.shape[0], "trial count mismatch between PSD and y"
            assert not np.isnan(PSD).any(), "PSD contains NaN"
            assert not np.isinf(PSD).any(), "PSD contains Inf"
            assert PSD.ndim == 3, f"expected 3D PSD features (n_trials, n_bands, n_channels), got ndim={PSD.ndim}"

            views = build_views(PSD)
            print(f"  PSD shape={PSD.shape}, y shape={y.shape}, views={ {n: v.shape for n, v in views.items()} }")
